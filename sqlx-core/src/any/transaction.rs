...
impl TransactionManager for AnyTransactionManager {
    ...
    fn begin(conn: &mut AnyConnection) -> BoxFuture<'_, Result<(), Error>> {
        match &mut conn.0 {
            ...
            AnyConnectionKind::Odbc(conn) => {
                <crate::odbc::Odbc as Database>::TransactionManager::begin(conn)
            }
        }
    }
    ...
    fn commit(conn: &mut AnyConnection) -> BoxFuture<'_, Result<(), Error>> {
        match &mut conn.0 {
            ...
            #[cfg(feature = "odbc")]
            AnyConnectionKind::Odbc(conn) => {
                <crate::odbc::Odbc as Database>::TransactionManager::commit(conn)
            }
        }
    }
    ...
    fn rollback(conn: &mut AnyConnection) -> BoxFuture<'_, Result<(), Error>> {
        match &mut conn.0 {
            ...
            #[cfg(feature = "odbc")]
            AnyConnectionKind::Odbc(conn) => {
                <crate::odbc::Odbc as Database>::TransactionManager::rollback(conn)
            }
        }
    }
    ...
    fn start_rollback(conn: &mut AnyConnection) {
        match &mut conn.0 {
            ...
            #[cfg(feature = "odbc")]
            AnyConnectionKind::Odbc(conn) => {
                <crate::odbc::Odbc as Database>::TransactionManager::start_rollback(conn)
            }
        }
    }
}
